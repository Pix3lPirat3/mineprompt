module.exports = function(bot) {
  ...
  async function handler(sender, args) {
    ...
    if (args.length === 0) return echo(`[[;#FF5555;]You must specify an item to drop.]`).id();
    
    var item = bot.inventory.items().filter(i => i.name.includes(args[0]))[0];
    if (!item) return echo(`Unknown item "[[;#FF5555;]${args[0]}]", check your inventory.`).id();
    
    if (args.length === 1) {
    	console.log(item)
      bot.toss(item.type, null, item.count);
      return echo(`Dropping "[[;#FF5555;]${item.count}x ${item.name}]".`).id();
    }
